...
treeType search(char model[], treeType root){
    ...
    else if(vn_strcmp(model, root->key.fullname) == 0) 
        return root;
    else if(vn_strcmp(model, root->key.fullname) < 0) return search(model,root->right);
    else{
...
void insertNode(keyType x, treeType *root){
    ...
    else if (vn_strcmp(x.fullname , (*root)->key.fullname) < 0) insertNode(x, &(*root)->left);
    else if(vn_strcmp(x.fullname , (*root)->key.fullname) > 0) insertNode(x, &(*root)->right);
}
...
void deleteNode(keyType x, treeType *root){
    if(*root != NULL){
        if( vn_strcmp(x.fullname, (*root)->key.fullname) < 0) deleteNode(x, &(*root)->left);
        else if( vn_strcmp(x.fullname, (*root)->key.fullname)>0) deleteNode(x, &(*root)->right);
